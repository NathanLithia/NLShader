...
#if (defined ADV_MAT && defined NORMAL_MAPPING && !defined COMPBR) || (defined ALTERNATIVE_NETHER_PORTAL && defined COMPBR)
		...
		if (water + moddedfluidX > 0.5) {
			...
			newNormal = clamp(normalize(normalMap * tbnMatrix), vec3(-1.0), vec3(1.0));
				...
				if (water < 0.5) {
					...
					if (netherportal > 0.5) {
						emissive = 0.25, albedo.r *= 1.5, lightmap = vec2(0.0);
						albedo.a = clamp(pow(albedo.r, 16.0) * 12.0, 0.02, 0.65);

						#ifdef ALTERNATIVE_NETHER_PORTAL
